...
summary: Esporta i risultati delle query, le tabelle del dashboard, la landing page e altri dati in file .CSV per analisi esterne, reportistica e integrazione con altre applicazioni tramite elaborazione asincrona. 
...
[role="lead"]
Esporta i dati in file .CSV da query, widget di tabelle dashboard e tabelle di landing page per analisi esterne, reportistica e integrazione con altre applicazioni. Il processo di esportazione asincrono ti permette di continuare a lavorare mentre grandi set di dati vengono elaborati in background.
