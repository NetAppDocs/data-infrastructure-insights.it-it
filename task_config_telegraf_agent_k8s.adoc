...
NOTE: Gli aggiornamenti tramite pulsante non sono disponibili per le versioni dell'operatore precedenti alla 1.2057.0. Segui le istruzioni di aggiornamento manuale riportate di seguito per aggiornare alla versione più recente. Successivamente, gli aggiornamenti futuri potranno essere eseguiti utilizzando la funzione di aggiornamento tramite pulsante.
...
| Errori "ImagePullBackoff" rilevati | Questi errori possono verificarsi se hai un repository docker personalizzato o privato e non hai ancora configurato il Kubernetes Monitoring Operator per riconoscerlo correttamente. <<Utilizzo di un repository Docker personalizzato o privato,Per saperne di più>> informazioni sulla configurazione per repo personalizzati/privati. 
...
| Vedo messaggi di errore da Telegraf simili al seguente, ma Telegraf si avvia e funziona: E! [inputs.kubernetes] Errore nel plugin: https://<IP>>:<port>>/pods[] stato HTTP restituito 403 Forbidden | Questi messaggi potrebbero comparire se stai utilizzando una versione di Kubernetes inferiore alla 1.33. La risorsa RBAC "nodes/pods" utilizzata da telegraf non esiste in queste versioni. Consulta la sezione "For Kubernetes versions < 1.33" di operator-additional-permissions.yaml per le risorse RBAC richieste nelle versioni precedenti. 
|===
