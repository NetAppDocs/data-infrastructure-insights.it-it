...
| Errori "ImagePullBackoff" rilevati | Questi errori possono verificarsi se hai un repository docker personalizzato o privato e non hai ancora configurato il Kubernetes Monitoring Operator per riconoscerlo correttamente. <<using-a-custom-or-private-docker-repository,Per saperne di più>> informazioni sulla configurazione per repo personalizzati/privati. 
