...
summary: Esamina i monitor definiti dal sistema per le condizioni relative a metriche, log, attributi e raccolta di dati. 
---
= Monitor di sistema
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
Data Infrastructure Insights include monitor predefiniti per metriche, log, attributi e condizioni di raccolta di dati. I monitor di sistema disponibili dipendono dai data collector presenti nel tuo tenant. Per questo motivo, i monitor disponibili in Data Infrastructure Insights possono cambiare quando vengono aggiunti data collector o modificate le loro configurazioni.
...
I monitor definiti dal sistema sono costituiti da metriche o attributi e condizioni predefinite, nonché da descrizioni e azioni correttive predefinite, che non possono essere modificate. Puoi modificare l'elenco dei destinatari delle notifiche per i monitor definiti dal sistema. Per visualizzare le metriche o gli attributi, le condizioni, la descrizione e le azioni correttive, oppure per modificare l'elenco dei destinatari, apri un gruppo di monitor definiti dal sistema e fai clic sul nome del monitor nell'elenco.
...
* *CVO Infrastructure* e *CVO Workload Examples* includono i monitor corrispondenti per Cloud Volumes ONTAP.
* Lo stato predefinito varia a seconda del monitor. Consulta le tabelle qui sotto ed esamina un monitor prima di abilitare le notifiche.
...
| Recupero del volume persistente di Kubernetes non riuscito | Il volume non ha superato il recupero automatico. |  | Attenzione @ > 0 B 
...
| Crash Loop Backoff del pod Kubernetes | Pod si è bloccato e ha tentato di riavviarsi più volte. |  | Critico @ > 3 
...
| Job Kubernetes non riuscito | Il processo non è stato completato correttamente a causa di un arresto anomalo o riavvio del nodo, esaurimento delle risorse, timeout del processo o errore di pianificazione del pod. | Controllare i registri eventi di Kubernetes per individuare le cause degli errori. | Attenzione @ > 1 
...
| Volume scoperto | Informativo | Questo messaggio viene visualizzato quando viene rilevato un volume interno. 


| Volume modificato | Informativo | Questo messaggio viene visualizzato quando viene modificato un volume interno. 


| Nodo rilevato | Informativo | Questo messaggio viene visualizzato quando viene rilevato un nodo di archiviazione. 


| Nodo rimosso | Informativo | Questo messaggio viene visualizzato quando viene rimosso un nodo di archiviazione. 
...
[[attribute-monitors]]
=== Monitor degli attributi

I monitor degli attributi valutano gli attributi di configurazione segnalati per un oggetto. A differenza dei monitor delle metriche, non confrontano una metrica di serie temporali con una soglia numerica. Ciascuna delle condizioni seguenti genera un avviso di tipo _Warning_ quando l'attributo segnalato corrisponde a uno stato non conforme o non disponibile.

I precedenti monitor autonomi delle metriche di sicurezza di ONTAP Essentials sono stati sostituiti dai monitor consolidati degli attributi *ONTAP Cluster Misconfigured* e *ONTAP Storage Misconfigured*. I corrispondenti monitor CVO applicano gli stessi controlli a Cloud Volumes ONTAP.



==== Monitor degli attributi ONTAP

|===
| Nome del monitor | Gruppo | Stato predefinito | Condizioni | Azione correttiva 


| Configurazione errata del cluster ONTAP | Infrastruttura ONTAP | Attivo | Avvisi quando viene rilevata una o più di queste 12 condizioni di sicurezza: * AutoSupport HTTPS non è configurato * Web HTTP è abilitato * SSH non è protetto * Il banner di accesso è disabilitato * La comunicazione peer del cluster non è crittografata * L'emittente del certificato è autofirmato * L'amministratore locale predefinito è abilitato * FIPS è disabilitato * L'inoltro dei log non è crittografato * L'hashing della password MD5 è abilitato * Remote Shell (RSH) è abilitato * Telnet è abilitato | Esamina gli attributi interessati nell'avviso. Disabilita i protocolli non sicuri e l'hashing delle password, proteggi SSH e la comunicazione peer, abilita i controlli di sicurezza necessari e sostituisci i certificati autofirmati secondo la policy di sicurezza della tua organizzazione. 


| Configurazione errata dello storage ONTAP | Infrastruttura ONTAP | Attivo | Ricevi avvisi quando una VM di storage ONTAP ha la registrazione degli audit disabilitata, SSH non è protetto o il banner di accesso è disabilitato. | Abilita la registrazione degli audit delle Storage VM, proteggi SSH e configura un banner di accesso appropriato. 


| Il cluster ONTAP non dispone di dischi di riserva | Infrastruttura ONTAP | Attivo | Avvisi quando il cluster segnala che i dischi di riserva sono in esaurimento. | Esamina i dischi di riserva per nodo e aggiungi capacità di riserva compatibile per mantenere la protezione dei dati. 


| Servizio di storage ONTAP non disponibile | Esempi di carico di lavoro ONTAP | Attivo | Ricevi avvisi quando un servizio FCP o iSCSI abilitato non è disponibile su una Storage VM ONTAP. | Verifica lo stato della Storage VM, del protocollo e della LIF dei dati. Controlla i messaggi recenti di stato e degli eventi di ONTAP e ripristina il servizio SAN interessato. 


| Nodo ONTAP configurato in modo errato | Esempi di carico di lavoro ONTAP | In pausa | Avvisi quando Cisco Discovery Protocol (CDP) è disabilitato su un nodo ONTAP. | Verifica l'impostazione CDP del nodo e lo stato della porta di rete, quindi abilita CDP dove necessario per il rilevamento della rete e il monitoraggio della topologia. 
|===


==== Cloud Volumes ONTAP Monitor degli attributi

|===
| Nome del monitor | Gruppo | Stato predefinito | Condizioni | Azione correttiva 


| Cluster CVO configurato in modo errato | Infrastruttura CVO | Attivo | Applica i controlli di configurazione della sicurezza del cluster descritti per *ONTAP Cluster Misconfigured* a Cloud Volumes ONTAP. | Esamina gli attributi interessati e correggi le impostazioni del cluster segnalate come non sicure o non conformi. 


| Archiviazione CVO configurata in modo errato | Infrastruttura CVO | Attivo | Avvisi quando una Storage VM di Cloud Volumes ONTAP ha la registrazione degli audit disabilitata, SSH non è protetto o il banner di accesso è disabilitato. | Abilita la registrazione degli audit delle Storage VM, proteggi SSH e configura un banner di accesso appropriato. 


| Nodo CVO configurato in modo errato | Esempi di workload CVO | In pausa | Avvisi quando CDP è disabilitato su un nodo Cloud Volumes ONTAP. | Verifica le impostazioni CDP e di rete del nodo, poi abilita CDP dove necessario. 
|===


==== Monitor di configurazione NTP

I seguenti monitor NTP rimangono disponibili come monitor di sistema autonomi per ONTAP e Cloud Volumes ONTAP.

|===
| Nome del monitor | Piattaforma | Soglia | Descrizione del monitor | Azione correttiva 


| Nessun server NTP è configurato | ONTAP | Attenzione @ < 1 | Il cluster non ha server NTP configurati. Per garantire ridondanza e un servizio ottimale, NetApp consiglia di associare almeno tre server NTP al cluster. | Associa un server NTP al cluster utilizzando  `cluster time-service ntp server create -server <ntp_server>`. 


| Il numero di server NTP è basso | ONTAP | Attenzione @ < 3 | Il cluster ha meno di tre server NTP configurati. | Associa server NTP aggiuntivi al cluster utilizzando  `cluster time-service ntp server create -server <ntp_server>`. 


| CVO Nessun server NTP è configurato | Cloud Volumes ONTAP | Attenzione @ < 1 | Il cluster non ha server NTP configurati. | Associa un server NTP al cluster utilizzando  `cluster time-service ntp server create -server <ntp_server>`. 


| Il numero di server NTP CVO è basso | Cloud Volumes ONTAP | Attenzione @ < 3 | Il cluster ha meno di tre server NTP configurati. | Associa server NTP aggiuntivi al cluster utilizzando  `cluster time-service ntp server create -server <ntp_server>`. 
|===
...
| Link dell'iniziatore FC MetroCluster down | CRITICO | L'adattatore dell'iniziatore FC è difettoso. | 1. Assicurarsi che il collegamento dell'iniziatore FC non sia stato manomesso. 2.  Verificare lo stato operativo dell'adattatore dell'iniziatore FC utilizzando il comando "system node run -node local -command storage show adapter". 


| MetroCluster FC-VI Interconnect Link non funzionante | CRITICO | Il collegamento fisico sulla porta FC-VI è offline. | 1. Assicurarsi che il collegamento FC-VI non sia stato manomesso. 2.  Verificare che lo stato fisico dell'adattatore FC-VI sia "Up" utilizzando il comando "metrocluster interconnect adapter show". 3.  Se la configurazione include switch fabric, assicurarsi che siano cablati e configurati correttamente. 
