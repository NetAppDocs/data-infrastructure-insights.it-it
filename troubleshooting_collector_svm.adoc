...
'''
*Problema:* Dopo aver aggiunto o aggiornato un ONTAP SVM Data Collector (Workload Security), il tenant visualizza un avviso di overprovisioning o di violazione dell'abbonamento. La capacità di storage raw è aumentata anche se non è stato aggiunto nuovo hardware.

*Prova questo:* Lo stesso storage può essere conteggiato due volte: una volta da un collector Observability ONTAP cluster e un'altra volta da un collector Workload Security SVM sullo stesso cluster.

Confronta la capacità di storage raw riportata dai collector Observability ONTAP e dai collector Workload Security SVM. Per ogni collector SVM, verifica se il suo cluster padre è già monitorato da un collector Observability ONTAP. Se lo è, solo la capacità del cluster deve essere conteggiata ai fini dell'utilizzo dell'abbonamento.

Conferma se si è verificata di recente una migrazione di SVM tra cluster. Dopo che un SVM è stato migrato, le metriche di capacità potrebbero rimanere associate al cluster precedente finché Workload Security non riceve informazioni aggiornate dal collector.

Assicurati che l'ONTAP SVM Data Collector per l'SVM migrato sia nello stato RUNNING. Se serve, riavvia il collector così Workload Security può raccogliere le metriche di cluster e capacità corrette dopo la migrazione.

Se nel totale compaiono sia la capacità del cluster che quella dell'SVM, è probabile che la capacità venga conteggiata due volte. Questo succede spesso poco dopo aver aggiunto, aggiornato o riavviato un collector SVM, oppure dopo aver migrato un SVM tra cluster.

Se l'utilizzo continua a superare il limite del tuo abbonamento, apri una richiesta di assistenza NetApp indicando l'utilizzo dell'abbonamento prima e dopo l'avviso, e un elenco dei collettori di Observability e SVM con la loro capacità di storage raw segnalata.

{vuoto}

'''
*Problema*: Uno o più Data Collector risultano in stato Degradato anziché In esecuzione, anche se il servizio del collector è attivo.

*Prova questo*: SWS segnala "Degradato" quando una o più connessioni FPolicy su un nodo vengono disconnesse. Non è un'interruzione: il collector è ancora in esecuzione, ma gli eventi di audit da quei nodi potrebbero andare persi finché la connettività non viene ripristinata.

ONTAP stabilisce una connessione del canale di controllo FPolicy da ciascun nodo che dispone di un LIF dati operativo per l'SVM. Un nodo senza un LIF dati locale non può connettersi. Vedi:

* link:https://docs.netapp.com/us-en/ontap/nas-audit/node-fpolicy-server-communication-process-concept.html["Processo di comunicazione tra nodo e server ONTAP FPolicy esterno"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/fpolicy-external-fpolicy-servers-concept.html["Come funziona ONTAP FPolicy con i server FPolicy esterni"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/requirements-best-practices-fpolicy-concept.html["Requisiti, considerazioni e best practice per configurare ONTAP FPolicy"^]


Nell'interfaccia utente, individua il/i nodo/i disconnesso/i, quindi verifica se tali nodi dispongono di un LIF dati per l'SVM.

* Nessun LIF di dati sul/sui nodo/i elencato/i: ignora lo stato Degradato. ONTAP può mostrare FPolicy disconnesso con il motivo "Nessun LIF locale presente per connettersi al server FPolicy" quando non è presente alcun LIF di dati locale; questo è previsto. Vedi link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/Fpolicy_server_not_connected_due_to_no_local_lif_present_on_one_or_more_nodes["KB: Il server Fpolicy non è connesso a causa di "no local lif present""^]. Una futura versione di SWS smetterà di contrassegnare il collector come Degradato in questo caso.
* Data LIF presente sul/sui nodo/i elencato/i: Indaga sul motivo per cui FPolicy è disconnesso (LIF non disponibile / manca data-fpolicy-client, problemi di rete/firewall o riavvio/failover temporaneo).
