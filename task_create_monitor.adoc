...
** _Rilevamento anomalie_ monitora e segnala i cambiamenti nei modelli dei tuoi dati
** I monitor degli attributi segnalano le modifiche agli attributi di un oggetto, come lo spostamento di un oggetto in un nuovo data center
...
I monitor degli attributi avvisano automaticamente il tuo team quando le risorse vengono spostate tra data center, un pool di storage non è crittografato o si verificano modifiche agli attributi. Questo ti aiuta a mantenere le configurazioni di base e ad assicurarti che nulla sfugga al controllo durante nuove implementazioni o modifiche.

image:attribute_monitor_in_list.png["I monitor degli attributi vengono aggiunti ai tipi di monitor"]

Scegli semplicemente il tipo di oggetto e seleziona gli attributi che vuoi monitorare per le modifiche.
