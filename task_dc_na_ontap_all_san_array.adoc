...
summary: Servizio di raccolta dati per array All-SAN ONTAP 
---
= Raccolta dati array All-SAN (ASA) NetApp ONTAP
:hardbreaks:
:allow-uri-read: 


[role="lead"]
Questo raccolta di dati acquisisce dati di inventario, log EMS e prestazioni dai sistemi storage che eseguono ONTAP 9.16.0 e versioni successive utilizzando chiamate API REST.
...
NOTE: ASA R2 indica i modelli di ultima generazione della piattaforma di storage ONTAP ASA. Sono inclusi i modelli di array ASA A1K, A90, A70, A50, A30 e A20.

Per tutti i sistemi ASA di generazione precedente, utilizzare il link:task_dc_na_ontap_rest.html["ONTAP REST"] raccoglitore.
