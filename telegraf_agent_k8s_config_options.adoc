...
[role="lead"]
IL link:task_config_telegraf_agent_k8s.html#configuringcustomizing-the-operator["Operatore di monitoring Kubernetes"] Offre ampie opzioni di personalizzazione tramite il file AgentConfiguration. Configura limiti di risorse, intervalli di raccolta, impostazioni proxy, tolleranze e impostazioni specifiche per ogni componente per Telegraf, Kube-State-Metrics, raccolta di log, mappatura dei carichi di lavoro, gestione delle modifiche e altro ancora per ottimizzare il monitoraggio del tuo ambiente Kubernetes.
