...
summary: Tutorial dettagliato per creare una dashboard delle prestazioni delle macchine virtuali con tabelle, grafici e widget che confrontano le metriche di VM e storage. 
...
[role="lead"]
Impara a creare una dashboard completa per le prestazioni delle macchine virtuali con questo tutorial passo passo. Creerai widget che visualizzano i dati sulle prestazioni delle VM, confronterai le metriche di storage backend con le prestazioni frontend delle VM e analizzerai le relazioni tra la latenza delle VM e la domanda di I/O per ottenere migliori informazioni operative.
