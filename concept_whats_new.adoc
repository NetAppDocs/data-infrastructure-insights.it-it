...
=== Visualizzazioni dell'assistente AI

Trasforma istantaneamente dati complessi in informazioni grafiche. Chiedi all'Assistente AI di creare una dashboard e visualizzerà la risposta con grafici e tabelle interattivi, aiutandoti a identificare le tendenze e risolvere i problemi più velocemente senza creare manualmente i grafici.

image:ai_assistant-column_graph.png["Visualizzazione dell'assistente AI"]



[[enhanced-vm-analyzer-filtering]]
=== Filtro avanzato di VM Analyzer

Una nuova opzione nella vista VM Analyzer ti permette di nascondere i volumi montati senza datastore, offrendo una visualizzazione più chiara per semplificare la risoluzione dei problemi delle VM.

image:vm_analyzer_datastore_only_view.png["cursore di visualizzazione solo datastore"]



[[ui-update-user-data-edit-mode]]
=== Aggiornamento dell'interfaccia utente: Modalità di modifica dei dati utente

Mantieni accurate le informazioni sugli asset riducendo il rischio di modifiche involontarie. Annotazioni e attributi (cioè "User Data") ora vengono visualizzati in modalità di sola lettura per impostazione predefinita, così puoi prima esaminare i dettagli e scegliere quando sei pronto a modificarli.

Modifica solo quando ne hai bisogno. Da un pannello a scomparsa, espandi i dettagli della risorsa e seleziona l'icona della matita. La landing page della risorsa si apre in modalità Modifica, dove puoi aggiornare le informazioni e selezionare Salva quando hai finito.

Puoi anche aprire direttamente la landing page della risorsa e selezionare Modifica nell'angolo in alto a destra.

image:UserDataEdit-Slideout.png["Modifica i dati utente nel pannello a scomparsa - icona a forma di matita, larghezza=400"]

image:UserDataEdit-LP.png["Modalità di modifica della landing page, larghezza=200"]
...
=== Nuova pagina iniziale dell'assistente AI

L'Assistente AI di DII ora include una pagina iniziale dedicata, che mette in primo piano l'analisi intelligente dell'infrastruttura. Avvia nuove conversazioni all'istante, riprendi la tua ultima sessione con un solo clic o inizia rapidamente con i prompt suggeriti.

image:AI_dashboard_page_default.png["Dashboard AI predefinita, larghezza=800"]

Accedi alla pagina iniziale da *AI Assistant* nel menu di navigazione oppure apri l'AI Assistant da qualsiasi posizione utilizzando l'icona "stars" in alto a sinistra.

image:AI_dashboard_page_menu.png["Menu di navigazione dell'assistente AI, larghezza=600"] image:AI_assistant_access_button.png["Accesso all'assistente AI da qualsiasi luogo, larghezza=300"]
...
=== Condividi aggiornamenti importanti con i messaggi di accesso

Mantieni il tuo team informato e allineato con messaggi di accesso personalizzabili. Comunica annunci importanti, programmi di manutenzione o aggiornamenti delle policy direttamente agli utenti quando effettuano l'accesso. Con il supporto Markdown, puoi formattare i messaggi per chiarezza e assicurarti che le informazioni critiche raggiungano ogni utente DII. Gli account con privilegi di amministratore possono trovare la configurazione nel menu Admin sotto Notifiche.



[[more-workspace-less-clutter]]
=== Più spazio di lavoro, meno disordine

Il menu di navigazione di DII ha un aspetto completamente rinnovato, ora in linea con i colori e il comportamento della Console per un'esperienza più uniforme. Il menu principale, precedentemente statico, sarà compresso per impostazione predefinita, offrendo un'area di lavoro più focalizzata e un utilizzo più efficiente dello spazio sullo schermo. Per accedervi, basta cliccare sull'icona del menu (☰) nell'angolo in alto a sinistra.



[[expand-device-visibility-with-intelligent-ai-assistance]]
=== Amplia la visibilità del dispositivo con l'assistenza intelligente dell'AI

Accelera l'integrazione di nuovi dispositivi con l'identificazione basata sull'intelligenza artificiale. Data Infrastructure Insights analizza il tuo ambiente e suggerisce regole di risoluzione automatica dei dispositivi, aiutandoti a identificare e gestire rapidamente un maggior numero di dispositivi. Ti basterà rivedere e approvare le regole generate dall'IA e DII si occuperà del resto, migliorando la visibilità e la gestione dei dispositivi in tutta la tua infrastruttura.



[[a-smarter-ai-assistant-for-global-teams]]
=== Un assistente AI più intelligente per i team globali

L'Assistente AI ora supporta più di una dozzina di lingue, consentendo al tuo team di interagire in modo naturale nella lingua preferita. Consulta l'elenco completo link:https://platform.claude.com/docs/en/build-with-claude/multilingual-support["Qui"^].



[[new-monitors-for-infrastructure-attribute-changes]]
=== Nuovi monitor per le modifiche agli attributi dell'infrastruttura

Ora sono disponibili i Monitor degli attributi per avvisare automaticamente il team quando le risorse vengono spostate tra data center, un pool di storage non è crittografato o si verificano modifiche agli attributi. Questo aiuta a mantenere le configurazioni di base e a garantire che nulla sfugga al controllo durante nuove implementazioni o modifiche.

image:attribute_monitor_in_list.png["I monitor degli attributi vengono aggiunti ai tipi di monitor"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== Crea avvisi e webhook più intelligenti con messaggi dinamici e ricchi di contesto

Quando crei un monitor o un webhook, è sufficiente digitare "%" nella descrizione dell'avviso o nell'URL del webhook, nell'intestazione o nel corpo per selezionare da un elenco popolato dinamicamente di attributi e annotazioni disponibili. Fornisci ai tuoi avvisi il contesto corretto per migliorare la chiarezza, ottimizzare l'integrazione con i webhook e velocizzare i tempi di risposta.

image:attribute_list_example.png["elenco attributi nella descrizione di un monitor, larghezza=400"]
...
=== DII conosce il tuo ambiente meglio che mai

Abbiamo migliorato il modo in cui l'IA di Data Infrastructure Insights comprende la tua configurazione. Ora si basa su un contesto più ricco relativo al tuo utilizzo attuale del servizio, alla configurazione e ai modelli di prestazioni prima di fornire una risposta. Il risultato è una guida iniziale che rispecchia il tuo ambiente anziché un punto di partenza generico, con un minor numero di verifiche necessarie per ottenere una risposta utile.

Ad esempio, accedi a un dispositivo di storage e l'Assistente AI fornisce risposte con un solo clic a domande come:

* Qual è l'andamento del tasso di utilizzo della capacità di questo storage e quali StoragePools stanno occupando più spazio?
* Potresti mostrarmi l'andamento dell'utilizzo della capacità di questo dispositivo nelle ultime 24 ore?
* Quali volumi occupano più spazio su questo storage?
...
=== AI Assistant è ora generalmente disponibile

L'assistente AI Data Infrastructure Insights è ora generalmente disponibile per tutti i clienti dei siti DII ospitati su AWS.

Tieni presente che l'AI Assistant su Azure sites è ancora in Preview. Controlla questa pagina per l'annuncio della disponibilità generale.



[[infrastructure-insights-features-removed]]
=== Funzionalità di Data Infrastructure Insights rimosse

Le funzionalità "Infrastructure Insights" di DII, _Shared Resources Under Stress_, _Kubernetes Namespaces Running out of Space_ e _Reclaiming ONTAP Cold Storage_, non sono più disponibili in DII.
...
=== Risposte intelligenti immediate sulla tua infrastruttura

Scopri il nuovo partner intelligente della tua infrastruttura: il link:ai_for_dii_overview.html["*Data Infrastructure Insights Assistente AI*"] trasforma il modo in cui interagisci con i tuoi dati, trasformando query complesse in semplici conversazioni. Scopri all'istante schemi nascosti, accelera la risoluzione dei problemi e prendi decisioni basate sui dati con sicurezza. Ottieni risposte immediate a domande come:
...
L'anteprima di DII AI Assistant è ora disponibile per la maggior parte dei clienti con servizi ospitati nelle regioni AWS; la regione Azure sarà disponibile a breve. link:concept_preview_features.html["Saperne di più"] sulle funzionalità di anteprima di Data Infrastructure Insights.

image:ai_assistant_icon.png["Icona dell'assistente AI"]
...
==== Personalizza i monitor di sistema più velocemente, senza duplicare

Modifica direttamente i monitor di sistema predefiniti in base alle tue esigenze: modifica nomi, filtri e raggruppamenti in un unico posto. Non è necessario creare monitor duplicati per semplici modifiche. Un utile banner ti ricorda quali monitor sono stati personalizzati, mantenendo la piena visibilità sulla configurazione del monitoraggio.



==== Semplifica la gestione dei monitor con azioni in blocco

Gestisci più monitor contemporaneamente tramite Bulk Actions. Regola rapidamente le impostazioni di Health Monitoring per gruppi di monitor, risparmiando tempo e riducendo la complessità. Aggiorna la tua strategia di monitoraggio in pochi secondi, non in minuti.



==== Concentra i punteggi di integrità della tua infrastruttura su ciò che conta di più

Escludi oggetti specifici dai punteggi di Infrastructure Health per mantenere accuratezza e pertinenza. Quando visualizzi la dashboard di Infrastructure Health, rimuovi immediatamente gli oggetti che desideri deprioritizzare senza modificare la configurazione del monitor. Questo concentra gli avvisi sugli elementi dell'infrastruttura che guidano le priorità aziendali.

image:health_monitoring_exclude_object.png["escludi l'oggetto dal monitoraggio dello stato di salute"]
...
I seguenti sistemi operativi Linux sono attualmente supportati con Data Infrastructure Insights link:concept_acquisition_unit_requirements.html["Unità di acquisizione"]:

* AlmaLinux 9.5 e 10
* Centos (64-bit): Stream 9
* Debian (64-bit): da 11 a 13
* OpenSUSE Leap 15.2 attraverso 15.6, 16
* Oracle Enterprise Linux (64-bit): 8.10, 9.2, 9.4, 9.6, 10
* Red Hat (64-bit): 8.10, 9.2, 9.4, 9.6, 10
* Rocky 9.2 fino a 9.6
* SUSE Linux Enterprise Server 15 SP4 through 15 SP7, 16
* Ubuntu Server: 20.04, 22.04, 24.04 LTS
* SELinux sulle piattaforme sopra
...
=== Controllo dell'accesso alle query con query condivise

Condividi le query che crei con controlli di autorizzazione dettagliati. Selezionare la modalità Visualizzatore di sola lettura per impedire modifiche o la modalità Modifica per consentirle. Le query private sono accessibili ai creatori, mentre le query condivise possono essere sfruttate nelle regole di annotazione o duplicate per casi d'uso specializzati, offrendoti il controllo completo su come le query vengono utilizzate e modificate all'interno della tua organizzazione.

image:query_sharing_dialog.png["impostazione dei permessi su una query, larghezza=200"]

Inoltre, gli utenti con ruolo _Ospite_ possono ora creare e gestire le proprie query private.



[[better-filtering-when-managing-monitors]]
=== Filtraggio migliore durante la gestione dei monitor

Il filtraggio per gli elenchi e i gruppi di Monitor è stato migliorato per aggiungere il supporto dei caratteri jolly, filtri multipli, funzionalità di inclusione/esclusione e altro ancora.

image:monitor_list_filters.png["Nuovo filtro monitor, larghezza=500"]



[[cleaner-dashboard-visualizations]]
=== Visualizzazioni della dashboard più pulite

Il design migliorato della dashboard ti aiuta a vedere ciò che conta di più. Maggiore chiarezza visiva con bordi ombreggiati che separano i widget e linee di separazione rimosse per ridurre l'ingombro e consentirti di concentrarti sulle informazioni sui dati. Questi perfezionamenti creano un'esperienza di dashboard più pulita e intuitiva, che consente di individuare tendenze e anomalie in modo più rapido e semplice.
...
Reclaim ONTAP Cold Storage Insight ora supporta FlexGroups ed è ora disponibile per tutti i clienti.
...
| Gli spazi dei nomi di Kubernetes stanno esaurendo lo spazio | L'analisi _Kubernetes Namespaces Running Out of Space_ ti offre una panoramica dei carichi di lavoro sui namespace Kubernetes che rischiano di esaurire lo spazio, con una stima del numero di giorni rimanenti prima che ogni spazio si riempia. 


| Risorsa condivisa sotto stress | La funzionalità di analisi _Shared Resource Under Stress_ utilizza l'intelligenza artificiale e l'apprendimento automatico per identificare automaticamente i punti in cui la contesa delle risorse sta causando peggioramento delle performance nel tuo ambiente, evidenzia i carichi di lavoro interessati e fornisce azioni correttive consigliate, permettendoti di risolvere i problemi di performance più rapidamente. 
...
Cloud Insights mette regolarmente in evidenza una serie di nuove e interessanti funzionalità in anteprima. Se sei interessato a visualizzare in anteprima una o più di queste funzionalità, contatta il tuo team di vendita NetApp per ulteriori informazioni.
...
| Risorsa condivisa sotto stress | La funzionalità di analisi "Risorse condivise sotto stress" utilizza l'intelligenza artificiale e l'apprendimento automatico per identificare automaticamente i punti in cui la contesa delle risorse sta causando peggioramento delle performance nel tuo ambiente, evidenzia i carichi di lavoro interessati e fornisce azioni correttive consigliate, consentendoti di risolvere i problemi di performance più rapidamente. 
|===
...
'''nt1.png[Gestione unità widget tabella]
