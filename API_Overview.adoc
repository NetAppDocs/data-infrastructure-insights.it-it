...
* Il tipo ASSETS contiene asset, query e API di ricerca. Gli attributi e le metriche degli asset DII sono talvolta denominati “odata”. Le API per questi sono in genere accessibili tramite /rest/v1/asset o /rest/v1/query. Vedere il <<api-documentation-swagger,Documentazione API Swagger>> per ulteriori informazioni.
+
...
* Il tipo DATA INGESTION viene utilizzato per recuperare e gestire dati di ingestione e metriche personalizzate, ad esempio dagli agenti Telegraf. Queste sono talvolta denominate metriche “datalake” o “lake”. Le API per queste sono in genere accessibili tramite /rest/v1/lake. Vedere il <<api-documentation-swagger,Documentazione API Swagger>> per ulteriori informazioni.
...
Si noti che i tipi di API a cui un utente ha accesso dipendono anche dal link:concept_user_roles.html["Ruolo utente"] che possiede in ciascun set di funzionalità di Data Infrastructure Insights (Observability, Workload Security, Reporting).
